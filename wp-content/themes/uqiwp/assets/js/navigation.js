...
(function($) {
    ...
    var uqiwpMenu = $('.uqiwp-inline-menu');
    $('.uqiwp-inline-menu li').find('> .sub-menu').before('<span class="mb-toggle js-show-mb-nav"><i class="fas fa-chevron-down"></i></span>'); // Desktop Menu

    var desktopMenu = function desktopMenu() {
        if ($(window).width() > screenWidth) {
            var uqiwpMenuLink = $('.uqiwp-inline-menu li');
            uqiwpMenuLink.find('> ul.sub-menu').addClass('hide');
            uqiwpMenuLink.on('mouseover', function() {
                $(this).find('> ul.sub-menu').addClass('open').removeClass('hide').stop(true, true);
            });
            uqiwpMenuLink.on('mouseleave', function() {
